-- ============================================================
-- 20260727000007_category_filter_includes_subcategories.sql
-- Los productos ahora se etiquetan con subcategorías (categories.
-- parent_id no nulo), pero el feed público filtra por el rubro
-- top-level (chip). p_category_id ahora matchea tanto la categoría
-- exacta del producto como cualquier subcategoría de ese rubro.
-- ============================================================
  ...
  where p.is_active = true
    ...
    and (
      ...
      or exists (
        ...
        where c.id = p.category_id and c.parent_id = p_category_id
      )
    )
    and (
      p_search is null
