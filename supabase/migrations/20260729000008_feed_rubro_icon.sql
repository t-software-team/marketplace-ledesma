-- Feed: return the shop's rubro slug so the client can show the matching
-- category icon (e.g. scissors for peluquerías) on each product card.

drop function if exists public.get_products_feed(
  double precision, double precision, uuid, text, integer, integer, text, text
);
