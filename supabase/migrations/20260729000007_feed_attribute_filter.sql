-- Feed: filter by a product attribute value (talle/color/año/etc.) and
-- return each product's attribute values so the feed card can show badges.
-- Dropped first: adding an output column isn't allowed via CREATE OR REPLACE
-- when the function returns a TABLE with a different column set.

drop function if exists public.get_products_feed(
  double precision, double precision, uuid, text, integer, integer, text
);
