...
/** Forwards next/email (e.g. from an invite link) into the "Ingresá" hop so
 * someone who already has an account doesn't lose that context. */
function loginQuery(searchParams: ReturnType<typeof useSearchParams>) {
  ...
  async function onSubmit(values: RegisterFormValues) {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        emailRedirectTo: `${window.location.origin}/auth/confirm?type=signup&next=${encodeURIComponent(next ?? '/onboarding')}`,
      },
    })
  ...
  return (
    <div className="space-y-8">
      ...
      <p className="text-center text-sm text-muted-foreground">
        ¿Ya tenés cuenta?{' '}
        <Link
          href={`/login${loginQuery(searchParams)}`}
