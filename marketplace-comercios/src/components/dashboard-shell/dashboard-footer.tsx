...
const SUPPORT_URL = `https://wa.me/${SUPPORT_PHONE}?text=${encodeURIComponent('Hola, necesito ayuda con Proxi Marketplace')}`
    ...
    <footer className="border-t border-border bg-surface px-4 py-4 md:px-6">
      <div className="mx-auto flex w-full max-w-5xl flex-col items-center justify-center gap-2 text-xs text-muted-foreground sm:flex-row sm:gap-4">
        <span className="flex items-center gap-1.5">
          <Store className="size-3.5" aria-hidden />© {new Date().getFullYear()} Proxi Marketplace - Christian
          Toscano
        </span>
        <a
          href={SUPPORT_URL}
