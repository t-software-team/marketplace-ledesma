-- get_products_feed paginates with OFFSET over a seeded random order. The
-- final ORDER BY tiebreaker was `random()`, which re-evaluates on every call
-- and can reorder rows that tie on every prior key, causing the same product
-- to shift between pages (duplicates or skips) as the client scrolls.
-- Replace it with `p.id`, a fully deterministic tiebreaker.
