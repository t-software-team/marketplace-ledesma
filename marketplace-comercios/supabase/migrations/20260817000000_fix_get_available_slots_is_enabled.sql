-- Fix: get_available_slots leía is_enabled (boolean) dentro de una variable
-- jsonb, lo que rompía con "invalid input syntax for type json" al castear
-- el booleano a texto ('f'/'t', no JSON válido).
...
begin
  ...
  loop
    ...
    v_slot_start := (p_date || ' ' || v_range_open) AT TIME ZONE v_timezone;
    v_slot_end := v_slot_start + make_interval(mins => v_duration_minutes);

    while v_slot_end <= ((p_date || ' ' || v_range_close) AT TIME ZONE v_timezone) loop
      if not exists (
