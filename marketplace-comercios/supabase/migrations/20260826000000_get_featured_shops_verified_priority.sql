-- Agrega verification_status = 'verified' como criterio de orden en
-- get_featured_shops, entre el precio del plan pago y el rating: dentro
-- del mismo grupo de plan, un comercio verificado desempata antes de
-- mirar el rating.
-- Migración nueva (no ALTER de un archivo ya aplicado) porque supabase
-- db push trackea migraciones por nombre de archivo, no por contenido.
